...
from utils import (
    ...
    get_forecasting_dataloader_qg_half,
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        if c.sample_only:
            ...
            if c.dataset == 'cifar':
                ...
                if '1st_half' in c.dataset:
                    self.train_loader, self.val_loader, old_pixel_norm, new_pixel_norm=get_forecasting_dataloader_qg_half(c)
                else:
    ...
    def prepare_batch_autoregressive(self, batch=None):
        ...
        steps = c.auto_step
        T = c.max_T
        assert not any(m % T == 0 for m in range(start, start + steps + 1)), f"Range [{start}, {start+steps}] contains a multiple of {T}"
